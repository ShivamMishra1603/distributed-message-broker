...
func TestManager_CreateAndReopenMetadata(t *testing.T) {
	...
	mgr, err := NewManager(dir, 1024*1024, 512*1024, 4096, "sync", nil, nil)
	if err != nil {
	...
	mgr2, err := NewManager(dir, 1024*1024, 512*1024, 4096, "sync", nil, nil)
	if err != nil {
...
func TestManager_CreateTopicValidation(t *testing.T) {
	dir := t.TempDir()
	mgr, _ := NewManager(dir, 1024*1024, 512*1024, 4096, "sync", nil, nil)
	defer mgr.Close()
...
func TestManager_CrashConsistency(t *testing.T) {
	...
	mgrWrite, _ := NewManager(dir, 1024*1024, 512*1024, 4096, "sync", nil, nil)
	...
	_, err = NewManager(dir, 1024*1024, 512*1024, 4096, "sync", nil, nil)
	if err == nil {
